...
Unit tests for the SOY domain — primarily the SQLAlchemy models, the
Alembic migration, and the JSONB / ENUM / FK invariants. These tests
are fast and require only the SOY package itself plus SQLAlchemy +
...
available (URL via ``SOY_TEST_DATABASE_URL``); if not, they fall back
